...
- [x] 2.1 Add the `private_key` keyword to `CA.issue_cert()`,
  ...
  from the authority.
- [x] 2.2 Raise `ValueError` when both `private_key` and `csr` are
  given. Verify with a `pytest.raises` test on the message.

## 3. Regression cover for existing behaviour

- [x] 3.1 Add a test issuing two certificates from one signing
...
- [x] 3.2 Add a test building an authority, intermediate and leaf
...
- [x] 4.1 Document the keyword on all three callables, including that
