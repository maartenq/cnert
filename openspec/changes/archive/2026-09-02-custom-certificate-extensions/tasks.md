...
- [x] 1.1 Rework `_CertBuilder.build()` to collect the built-in
  ...
  unchanged.
- [x] 1.2 Add the deduplication step that keeps the last pair per
  object identifier. Verify with a unit test feeding two pairs of the
  same type and asserting one survives.
- [x] 1.3 Give `_CertBuilder.build()` its `extensions` argument and
...
- [x] 1.4 Thread `extensions` through `CA.__init__` and
...
- [x] 2.1 Apply the same collect, append and deduplicate shape to
...
- [x] 3.1 Document the keyword on every touched public callable,
...
- [x] 3.3 Run `task check` and `task test` and confirm both are
  green.
